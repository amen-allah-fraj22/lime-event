...
**Update (2026-08-17): resolved, and the health check is now diagnosable.** Connectivity came back — direct Prisma `SELECT 1` succeeds and `/health/db` returns `connected` on repeated checks. The outage was external reachability of the Supabase pooler (`aws-1-eu-central-1.pooler.supabase.com:5432`, session mode), not a code fault. What *was* a code weakness: the `health/db` handler swallowed the underlying error with a bare `catch`, which is exactly why two sections couldn't tell *why* it was down. Fixed — it now logs the real Prisma error code (P1001 host unreachable / P1000 auth / P1017 server-closed) and message to the server log, and returns the `code` in the JSON, while keeping the response otherwise generic. Next time it flips to disconnected, the reason is one log line away instead of a guessing game.
...
1. **Supabase connectivity — ✅ resolved (2026-08-17).** The database had been intermittently unreachable (`GET /health/db` → 503) across sections 8 and 9. It's now up and stable — verified live: a direct Prisma `SELECT 1` succeeds, `/health/db` returns `connected` on repeated checks, and `/artists` returns real data (including a genuine non-seed artist). The outage was external reachability (a paused/cold pooler), not a code fault. Hardened the health check so a future recurrence is diagnosable instead of silent (logs the real Prisma error code + message; see section 8 note).
...
4. **Run the manual + real-device passes** (sections 3 & 4): full organizer and artist click-throughs on a real phone, now that the DB is up.
